...
| **M5.5** | ~~Methods on `struct` and `class`~~ **done**. ~~`unsafe` blocks (D35)~~ **done**. Then the rest of the runtime floor: `extern`, `alloc<T>`, and enough of `kl_rt` to malloc. | A `Buffer` with `push` that grows its own storage and frees it. | Whether the language can express a real data structure |
...
- The `unsafe` gate guards exactly **one** operation — the pointer-to-pointer
  conversion — so the feature is real but the list is not yet a list. `alloc<T>`
  is not on it (§12 never said whether allocation itself needs one), `extern`
  joins it in the next slice, and dereference is a decision still to take. Until
  there is a second entry, `unsafe_used_` is a flag with a single writer and the
  "uses none of its permission" rule has only one way to be satisfied.
