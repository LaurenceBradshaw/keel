...
| **M5.5** | ~~Methods on `struct` and `class`~~ **done**. Then the runtime floor: `extern`, `alloc<T>`, and enough of `kl_rt` to malloc. | A `Buffer` with `push` that grows its own storage and frees it. | Whether the language can express a real data structure |
