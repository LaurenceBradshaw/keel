...
a milestone rather than a place in the library's. **M9 is error handling** (2026-09-24): `try`,
`Result<T, E>` and §12's anonymous error union had been dated only as "after M6" and so had no
milestone at all once M6.5 closed — one of four deferrals a scheduling audit found aimed at a
phrase rather than a row, see §9. §15 is the live tracker.
...
and has no plan to: closures are on §6.6's not-in-v0 list, and M6 shipped without them (corrected
2026-09-24 - this said "until M6's lambdas", which named a delivery that was never scheduled). B10 measured `current_function_` and found `enter_function` called 2,972 times
...
with no enclosing function**, because the language has no nested functions and no plan for them -
§6.6 excludes closures (corrected 2026-09-24). Both
...
The first cut still refuses an owning payload. Destroying an enum means destroying the **active** variant, chosen at run time - a switch on the tag inside the destructor - and deferring that costs nothing anyone can use yet, because a variant worth owning needs allocation, which is M5.5. `Result<i32, Error_code>` works without it. **Deferred to M5.5 and missed by it, and deliberately not rescheduled (2026-09-24)**: M5.5's row names methods, `unsafe`, `extern` and `alloc`/`free` and not this, so it was owned by a milestone that closed. The answer is not a new date but a **trigger** - it stays refused until a real use case exists, because a synthesised tag-switching destructor is real work and nothing has yet needed one. **The use case to watch for is `Result<Buffer, E>`**, which M9's error handling makes writable; if the library reaches for it there, that is the signal, and until then the refusal costs nothing anyone can spend. |
...
**`Optional<T>` is on this list as a library, not as a language feature** (clarified 2026-09-24):
`enum Optional<T> { Some( T ), None }` needs payload-carrying variants and generics and nothing
else, and both shipped - M5 and M6. So it arrives with M8's library rather than needing a decision,
and "any standard library" at the end of the list is the entry that actually holds it back. Note it
does **not** give Keel an optional *reference*: `ref` is a binding mode rather than a type
constructor (D32, L16), so `Optional<const ref T>` has no spelling, and the nullable read-only
pointer gap belongs to the pointer-to-const decision in M8's `[*]T` slice instead.
...
**`[*]T` was the same gap, found the same way, and scheduled here (2026-09-24).** It appeared in
no milestone row and on no exclusion list in §6.6 — only inside D27 as *"which v0 does not
have"* — while **five** entries elsewhere named it as the event they defer to: D37's `alloc<T>()`
that takes no count, D35's `unsafe` list that stays at one operation, D34's fork between slicing
as a language operation and `Range` becoming a type, the missing spelling for a pointer to const,
and the escaping-raw-pointer hole. M5.5's row had already deferred *"the growable-array half to
M8"* and M8's row never picked it up, which is how a deferral with a named destination still
ends up owned by nobody. **It goes first inside M8 and on its own**, before modules and before the
library, for the reason the M5.5 gap teaches: `Vector<i32>` is M8's acceptance criterion and
`[*]T` is its prerequisite, so listing them in one row without an order is what let the deferral
evaporate the first time. Landing it alone also puts four of the five parked decisions in one
slice where they can be taken together, since the dereference gate, the pointer-to-const spelling
and the slicing fork all want the type to exist before they can be decided.

**Looking for the same shape found three more (2026-09-24).** The pattern is a deferral whose
destination is a phrase rather than a row: "which is M5.5", "post-M6", "at M5". A phrase reads like
a schedule, no milestone row ever picks it up, and the milestone then closes.

- **An owning enum payload.** D7 and D30 both defer it with *"a variant worth owning needs
  allocation, which is M5.5"*, and §15 calls it *"naturally M5.5's work"*. **M5.5 shipped without
  it** - its row names methods, `unsafe`, `extern` and `alloc`/`free`, and not this. **Decided: it
  stays refused, and the gate is a use case rather than a date.** Not every missed deferral wants a
  new milestone; this one is a synthesised tag-switching destructor, which is real work, and nothing
  has yet needed one. `Result<Buffer, E>` is the case to watch for and M9 is what makes it writable,
  so the signal will arrive on its own. Recording the trigger is the fix here, not rescheduling.
- **Error handling itself - `try`, `Result<T, E>` and the error union - had no milestone at all.**
  Two §12 rows dated it *"after M6"* and *"post-M6"*, written when M6 was the last row on the table.
  M6 and M6.5 are done, M7 is static methods, and M8 is `[*]T`, modules and the library, so the
  phrase pointed past the end of the plan. **Now M9**, a row of its own rather than a clause on
  someone else's, because it is milestone-sized: the anonymous error union is, in §12's own words,
  *"the largest type-system addition in this document"*.
- **`Optional` is a phantom, and the answer is that it was never a language feature.** §15
  explained the missing nullable read-only pointer as needing *"M5's `Optional`"*. M5's row is
  `enum` and `switch`, and §6.6 lists Optionals as not in v0. But `enum Optional<T> { Some( T ),
  None }` is an ordinary payload-carrying generic enum, and **both halves shipped** - payloads at
  M5, generics at M6 - so it needs no language work at all and belongs to M8's library beside
  `Vector` and `String`. §6.6's entry means "no library", which M8 is what changes.
  **What `Optional` does *not* fix is the gap that sentence was about.** Read-only in Keel is
  `const ref`, and D32 and L16 make `ref` a *binding mode rather than a type constructor*, so
  `Optional<const ref T>` cannot be written - there is no such type to pass as a type argument.
  The nullable half is library work; the read-only half needs pointer-to-const, which is the
  decision dated to M8's `[*]T` slice. Two different gaps in one sentence, which is how one of
  them hid behind the other. D37's `nullptr` stopgap had the same defect from the other end:
  *"replaced by `Result` at M5"*, where M5 shipped without `Result`; that one is now M9's.

**M9 goes after M8, and the plan already contains the argument (2026-09-24).** M6.5's row moved
function pointers out of M8 with the reason *"M8 is the library, and a language feature should not
arrive inside a milestone that is otherwise about writing Keel in Keel"* - and error handling is a
language feature far larger than function pointers, so the same rule puts it after rather than
inside. §12 wants it there independently: the evidence that settles the error-union question is
*"real error-handling code"*, and M8's `Vector` and `String` are the first real code there is to
judge it against. **The cost is accepted rather than hidden**: M8's library gets written against
D37's `nullptr` stopgap and is revised at M9, which is what the stopgap has always been for.

**The common failure is writing the destination in prose and not in the table.** Both halves are
cheap; only one of them is load-bearing. Every deferral above was honest about *why* it waited and
silent about *who* would do it, which is exactly how M5.5's methods went missing, and the fix each
time is a row rather than a sentence.
...
| **What else does a `class` have, and which of those does Keel want?** **Audited 2026-09-25**, when access control was scoped and the question “what is a class missing” had to be asked properly. Inheritance and dynamic dispatch are the large answer and have an entry of their own in §15; these are the five small ones, and every one of them is **a parse error by accident rather than a refusal by decision**, which is the thing worth fixing whatever the verdict. *Access granularity beyond the type* - `friend`, or a module-wide grant. M7's slice cannot avoid the small version of this, because the moment a field is private a free function is the first thing to lose sight of it. **Leaning: no `friend`, ever.** The module is the unit (§6.6), Keel's tests are golden files rather than in-source, and a type that needs a helper inside its own module can have a private static method - M7 and M8 supply both halves. *Default member initialisers*, `i32 x = 0;` in a class body. Today `expected ';', found '='`. **Leaning: not in v0**, and the reason is structural rather than a preference: a class is built by a constructor and a struct by a literal that names every field (D23, D29), so there is currently nowhere a default would be read from. It becomes worth having the moment a constructor may leave a field alone. *Constructor member-initialiser lists*, `C( i32 v ) : x( v ) { }`. **Leaning: never**, and this one is a real difference from C++ rather than a gap: C++ needs the list because of const members, reference members and base initialisation, and Keel has none of the three - `ref` is a binding mode rather than a member type (D32, L16), `const` binds a name, and there are no bases. Assignment in the body is not second-best here, it is the whole thing. Written down because its absence looks like an oversight. *Nested types and member type aliases*. Today the diagnostic is actively wrong - `struct Inner { };` inside a class body reports “`struct` is a keyword, so it cannot be used as a name” and points at the opening brace. **Leaning: no**, since the module is the namespace and a nested type buys only a qualified name, but the diagnostic is worth fixing while access control is already in that member loop. *Duplicating a `class`*. §6.6 bans copy constructors, D29 says a class is moved rather than copied, and `Copyable` means **trivially** copyable - so today a class can be moved and never duplicated, and nothing says whether that is the design. **It is, and this is where it gets written down: an explicit `clone()` method**, an ordinary method needing nothing from the language. The entry exists so an intentional absence is not mistaken for a missing feature. | **M7** for the access-granularity half, which its slice cannot avoid; the other four **M8**, when the library is the first code to write real classes |
...
which needs ~~M5's `Optional`~~ **two separate things this sentence conflated (corrected
2026-09-24)**. `Optional<T>` is an ordinary payload-carrying generic enum and needs no language
work - payloads shipped at M5, generics at M6 - so it is M8 library work, not a deferral.
**But it does not close this gap**, because read-only here is `const ref`, and D32 and L16 make
`ref` a binding *mode* rather than a type constructor, so `Optional<const ref T>` has no spelling.
The read-only half needs pointer-to-const, which is dated with `[*]T` in M8's first slice, as is
the read-only buffer above. Both gaps land on features already deferred, which is what
made the decision cheap - **and the `Optional` half was never deferred at all**, which is the part
this sentence got wrong. **`[*]T` is the trigger to revisit it**, and it is **scheduled as M8's first slice (2026-09-24)**: it is the first
...
### M7 slice: the tests, before the feature (2026-09-24)

**Written first, and red on purpose.** Fifteen unit `TEST_CASE`s and four golden fixtures describe
static methods; none of the feature exists. The unit suite reports 663 cases, 649 passing and 14
failing, and the 649 is the 647 that passed before plus two of the new ones. The golden suite is
176 passing and 4 failing, all four of them the new fixtures. **No existing test changed
behaviour**, which is what makes the red meaningful rather than noise.

The two that pass are guards rather than accidents: `::` reaching a variant still works, and a
qualifier that reaches nothing - a primitive, an expression - is still refused. Both are what M7
widens, so a widening written as a replacement rather than as an extra arm is what they catch.

**One of them passed for the wrong reason and was tightened.** "A static method is refused through
an object" was asserting only that the program is refused, which is true today for the unrelated
reason that `static` does not parse. It now counts the type errors and requires exactly one, so it
can only pass once the declaration is understood. Worth recording as a class rather than a fix: a
refusal test written before the feature will pass vacuously unless it also asserts that everything
*around* the refusal was accepted.

**The suite found three things the walkthrough had not.**

*"Does this declaration have a receiver" is derived from the node kind in four places, not two.*
`Places::receiver_of` (`sema/places.cpp:202`) and the lowerer's parameter walk (`ir/lower.cpp:228`)
were the two already known. `is_const_method` (`sema/type_checker.cpp:259`) guards on `params` being
non-empty and then asks parameter 0, so a static method whose first written parameter is a `const
ref` reads as a `const` method. And `Overloads::parameters_collide` (`sema/overloads.cpp:724`) picks
how many parameters to skip with `kind == Function_decl ? 0 : 1`, so it compares the wrong lists.
Each of the four answers wrongly and silently rather than failing.

*A static and an instance method of one name and one written parameter list emit one C symbol.* The
category tag names the enclosing type, and the receiver is among the parameters in neither - so
`static i32 f( i32 )` and `i32 f( i32 ) const` on the same type mangle alike. No call site is
ambiguous, since the spellings differ, but `cc` refuses two definitions of one name: a failure in
generated code rather than a diagnostic, which is the worst class this project has. **This is the
collision M6.5 closed for methods against free functions, reopened by a member with no receiver to
encode**, and it is the direct consequence of the `parameters_collide` defect above. **Proposed
rule: refuse the pair**, which is what C++ does and which keeps the scheme injective with no change
to it. The tests encode that rule; it is a decision rather than a discovery and is open until taken.

*`Box<i32>::of( 7 )` does not parse.* A type-argument list in expression position is recognised only
when a `(` follows it - `scan_type_arguments` (`parse/parser.cpp:1700`) ends on that test, which is
true of `id<i32>( 1 )` and false of every scoped spelling. So the qualifier breaks before the call
is reached. It matters because `Vector<i32>::with_capacity( n )` is the shape M8's first customer is
written in, and because the type arguments are the only place a static call can read them from,
there being no receiver to carry them. Three sections pin the boundary: the scoped spelling must
parse, `id<i32>( 1 )` must stay a generic call, and `a < b` must stay a comparison.

**A static call is a new monomorphisation seed**, which §12 already flags for `&f` in the function
pointer row. Nothing about it passes through a receiver, so the instantiation is recorded at the
call or not at all - and a miss is a symbol referenced and never defined, reported by `cc`. Two
tests cover it: the checker records the instantiation, and the emitter writes the instance's body.

**What the fixtures cannot assert yet.** A golden's `.expected` and `.stderr` record what the
compiler does, not what it should - so the four `.kl` files are written and their expectations are
not. They are generated with `run_tests.sh --update` as each slice lands, and the diff is read
before it is committed. Until then the golden suite is red by four, which is the cost of the rule
and is worth naming: an unrelated regression arriving in that window shows up as a fifth failure
rather than as a change of colour.

**The spelling decision is still §12's and is still open.** Every test above assumes the leaning -
a `static` keyword preceding the return type - because a test has to assume something. Changing the
answer changes the fixtures and nothing else in what was found.


### M7 slice: static methods - done (2026-09-25)

**Green on both suites**: 663 unit cases, 6306 assertions, and 180 goldens. The spelling decision
was taken as §12 leaned - a `static` keyword before the return type - and the tests written the day
before needed no revision to it.

**`has_receiver` is the predicate, not `is_static_method`.** The first attempt read the node kind's
replacement the wrong way round and broke three passing tests plus aborted the run: a free function
has no receiver either, so substituting "is this a static method" for "is this a Function_decl" made
`receiver_of` hand back a free function's first parameter and made the lowerer adopt it as `this`.
The question every one of those sites is really asking is *does parameter 0 exist and is it the
synthesised `this`*, which is exact because `this` is a keyword no written parameter can carry. So
`has_receiver( ast, decl )` is the primitive and `is_static_method` is defined from it. Five callers
read it: `Places::receiver_of`, the lowerer's parameter walk, `is_const_method`,
`Overloads::parameters_collide`, and `check_method_arguments`, which had `.subspan( 1 )` written in.

**Written down because it is the shape of the whole slice**: every defect found here was a question
about a *signature* answered by looking at a *node kind*. The fix is the same each time and the
diagnosis was the same each time.

**A path now carries its own type arguments.** `Box<i32>::of( 7 )` did not parse: a type-argument
list in expression position was recognised only when a `(` followed it. Widening the scan was half
of it - the branch then expected a call and found `::`. So `Path_expr` gained a second child, the
`Type_arg_list`, and the trailing `( 7 )` is an ordinary call over the path. That second child is
**the only place a static call can learn its instance**, there being no receiver to read it off,
which is why it belongs on the path rather than on the call.

**The collision was real and is refused.** A static and an instance method of one name and one
written parameter list mangle alike, and the `has_receiver` fix to `parameters_collide` made the
pair collide and be reported without another line being written. The *note* was wrong, though:
`is_const_method` differed between them, so the existing clause blamed the trailing `const` - and
removing it would have changed nothing. A clause above it now names the real difference. Worth
recording as a class: a diagnostic reached by the right branch for the wrong reason still passes
every test that only counts errors.

**What check_method_arguments already did.** The generic case cost almost nothing, which was not
expected: a static call hands the *qualifier's* type where a method call hands the receiver's, and
from there the bindings, the parameter substitution and `record_method_instantiation` are the same
code. §12's silent-miss failure class - a generic instantiation seeded by nothing and referenced by
a symbol never emitted - never arose, because the seed was recorded by the function that already
records it for methods.

**Two things deliberately not built.** A generic static call with *deduced* type arguments,
`Box::of( 7 )`, is refused naming what to write instead: deduction from arguments is wired for free
generic functions and not for a qualifier, and M8's customer writes `Vector<i32>::with_capacity`
rather than leaving it open. And `enclosing_aggregate` is a scan over the root's declarations rather
than a recorded edge - twice, once in sema and once in the mangler. Nothing has ever needed a
member's owner before, because every other question about a member goes through its receiver. If a
third caller appears it should become a map rather than a third scan.

**One golden outside the slice moved**, and it is an improvement rather than a cost:
`sema/errors_enums.kl` recorded *"`Point` is not an `enum`"* with the note *"`::` reaches a variant,
and only an `enum` has them"*. Both stopped being true. It now reads *"`Point` has no static method
`x`"* with *"`x` is a field, so it belongs to an object rather than to `Point`"*, which names the
actual mistake instead of the category.

**M7 still owes two of its four.** Access control is next by the milestone's own order, then
function pointers last because nothing depends on them.


### M7: static fields, checked and deferred to M8 (2026-09-25)

Asked whether static fields should land before the rest of M7. The answer is no, but the checking
turned up two things worth keeping, and one of them is a hole that is open today.

**The scheduling failure, first.** M7's row and the §15 debt beside it both deferred type-scoped data
to "M8's modules and globals". That names M8's *contents*, not M8's row, and M8's row never carried
it - so the feature was deferred to nowhere. The 2026-09-24 scheduling audit was run to catch exactly
this and found four; this is a fifth it missed, because the audit looked for things dated *"after
M6"* and *"post-M6"* and this one was dated by pointing at another milestone's prose. **A deferral is
only real when a row carries it**, which is the test to apply rather than the lesson to remember.

**The hole - found here, closed here (2026-09-25).** `static i32 count;` inside a class compiled
silently into an ordinary field, so every object got its own `count`. The member loop computed
`is_static` and then called `parse_field_decl()` with no arguments: the keyword was dropped rather
than refused. This was M7's own loose end rather than M8's - **the slice that introduces a keyword
owes a refusal for every member kind the keyword does not yet mean anything on.** A constructor and
a destructor each got one; a field did not, because nothing was looking at fields. It is now refused
at the keyword, naming what it would mean rather than what it is waiting for: *"it would be one
variable shared by every object, and that is not built yet"*. The field is still parsed afterwards,
so the members below it are checked and their errors reported.

**Mutation testing found a second gap, older than this one.** Four mutations of the guard; three
died at once. Dropping `!is_destructor` **survived both suites** - and it survived because *no golden
covered a static destructor at all*, which was true before this change too. The sibling refusal
shipped in the slice above with only a `has_errors()` unit test behind it, and the fixture's own
comment claimed to cover a constructor *and* a destructor while carrying only the constructor. The
fixture now carries both, the mutation dies, and the older rule gained the coverage it was written
to have. **A surviving mutation of new code found missing coverage of old code** - worth the pattern:
the mutation is testing the suite, not the change, and the suite is older than the change.

**The design, recorded so it is not re-derived.** A static field is a `Var_decl` in the aggregate's
member list, **not a `Field_decl` carrying a flag.** `Field_decl` is read at twenty-three places,
eighteen of them tests: struct layout, `contained_fields()`, the struct-literal all-fields rule, the
destructor's field sequence, the containment-cycle check, `Lowering::field_of`, `Aggregates`,
`Bounds`, `Places`. Under a flag every one needs a clause remembering to skip, and **each one that
forgets fails silently** - a static field in the emitted C struct, or one a struct literal demands be
initialised. As a `Var_decl` all eighteen skip it for nothing, because it is no longer a field. What
changes instead is the five sites that assume *a global is a child of the root*, and those fail
loudly: a name that does not resolve, a symbol the linker cannot find. **Trade a silent failure class
for a noisy one** - the same trade `has_receiver` made in the slice above, and for the same reason: a
question about *storage* answered by a *node kind* is fixed by changing the kind, not by adding a
flag beside it.

**The edge that will break first**, written down because it is invisible from either end.
`Statements::visit`'s default case recurses into every child and `Class_decl` has no case of its own,
so a `Var_decl` member would reach `visit_var` - the function-*local* path - be given a slot, and
skip the constant-initialiser rule entirely. The `Source_file` case picks `visit_global` by asking
what the *parent* is, and a static field gives that question a second true answer. `Resolver::visit`
already carries an explicit aggregate case for the same reason; `Statements` needs one too, routing
its members rather than falling through.

**What is genuinely M8-sized**, and the only part that is: a static field on a generic aggregate
needs one storage per instantiation, emitted by the monomorphisation worklist rather than by a walk
over the root. Everything else - declaring, folding, checking, emitting, mangling - already exists
for file-scope variables and would be reached rather than written. `mangle_local` suffixes the node
id, so two types each holding a `count` cannot collide without a line being added. That asymmetry is
why the deferral is right, and why M8's row says *after modules* rather than *with them*.
...
and five qualified accesses are refused from outside the type. Both suites are green at 6,347
assertions in 666 cases and 182 goldens, and all nine mutations were killed.

**The walkthrough that scoped this slice was wrong about where the work lives, and the correction
is the useful part.** It said access control "is a resolver feature - member lookup carrying
visibility", which M7's row had said since 2026-09-19. It is not. The resolver resolves *bare*
names, and a bare name that reaches a member is resolved inside that member's own type - always
visible, so nothing there can be refused. Every decision is about a **qualified** access, and all
five are in `sema/expressions.cpp`: `obj.f`, `obj.f( ... )`, `Type::f( ... )`, `C( ... )` and
`C { ... }`. The resolver's only part is having bound the type name already.

**The predicate's signature was the defect, and it is this project's own recurring one.** The
walkthrough wrote `is_visible_from( ast, member, from )` and described `from` as "the enclosing
aggregate", then at the call sites reached for `Places::receiver_of( current_function_ )` - which
returns a **parameter**, not an aggregate. Two incompatible meanings for one argument, in one
document. Implemented faithfully, that forces the caller to convert a parameter into a type, and the
first attempt did it by walking `ast.child( owner, 0 )` in a loop meant to reach a parent - a tree
with no parent pointers, so the walk descended until it asserted. It also never compared the
member's owner to `from` at all, so any access from inside *any* aggregate was allowed: one class
could read another's private fields.

**This is the same defect class as `has_receiver` and the static-field `Field_decl` analysis, and it
arrived through a design document rather than through code.** A question about one thing answered by
a node of another kind. The rule that falls out: *a predicate's parameter is named by what it is, and
every call site produces that thing by one route.* `from` is now the aggregate a function was
declared in, produced by `Expressions::current_type()` and nothing else - one expression, no branch
for static versus instance methods, because a static method is as much inside its type as an
instance one is and only the *receiver* framing ever suggested otherwise.

**`enclosing_aggregate` is now one function.** It existed twice - in the mangler and in the checker -
and the carried note said a third caller should turn it into a map. The third caller appeared; the
map did not, because correctness came first and the scan is not measurably hot. It moved to
`ast/ast.h` instead, below both, which is what stops the two copies disagreeing.

**The default was written the wrong way first, and the test suite said so.** Defaulting every member
of a `class` to private - which is what C++ does - broke 23 unit cases and 78 goldens, and the
failures were almost all *methods*: `add`, `twice`, `read`, `get`, `bump`. That is D29's principle
failing out loud. A class hides its representation, and a method is not its representation. Fields
only, and the churn fell to 19 cases. The remainder was real: 25 unit snippets and 14 golden
fixtures read a class field from outside, and each took an explicit `public`.

**The one thing to watch.** A `class` with no constructor can no longer be built from a literal,
because all of its fields are private - the diagnostic names the field and points at the
constructor. That was `codegen/destructors.kl`'s `Buffer`, `Wrapper` and `Handle`, and it is the
first time D29's *"a class is built by a constructor"* has had teeth rather than being a convention.

**A surviving-mutation note, the inverse of the last slice's.** Five mutations were killed only by
the golden while the unit suite passed, because the slice had shipped with no unit tests in the file
that implements it. Unit tests went into `sema/expressions.cpp` where the rules live; one mutation
...
**A scheduling audit ran on 2026-09-24 and its findings are in §9**, not here: four things were
deferred in prose to a destination no milestone row ever carried - `[*]T`, the owning enum payload,
error handling, and an `Optional` that is on the not-in-v0 list. Three are now scheduled and one is
...
- ~~**Error handling has no milestone.**~~ **Scheduled as M9 (2026-09-24).** `try`, `Result<T, E>`
  and §12's anonymous error union were dated *"after M6"* and *"post-M6"* in the only two places
  that dated them, and nothing on the table came after M8. It is a row of its own, after M8, on
  M6.5's own rule that a language feature does not arrive inside the milestone that writes the
  library. The prerequisite that is *not* dated is deliberate: `Result<Buffer, E>` needs an owning
  enum payload, which stays refused until something real wants one - M9 is what would make that
  something writable, so the trigger and the milestone are in the right order.
...
- ~~**Static methods have no spelling.** Every method takes a receiver, so a function that belongs to
  ...
  the entry below rather than this one. **Both deferrals were pointing at nothing (2026-09-25)**:
  "M8's modules" names that milestone's contents, not its row, and no row carried type-scoped data -
  the same failure the audit above was run to catch. **Static fields are now scheduled in M8's row**,
  after modules, with the design recorded in the entry above rather than left to be re-derived.
...
- **Inheritance and dynamic dispatch are planned, unscheduled, and named by no row.** L13 and D29
  both say *"planned and unscheduled"*, §6.6 lists them as not in v0, and §12's polymorphism row
  dated the dynamic half *"after M8, with real code"* - then M9 took that slot for error handling on
  2026-09-24, so the only date any of it ever had now names a milestone about something else. **The
  same failure again (2026-09-25)**: a deferral whose destination is a phrase rather than a row, and
  the largest one left. **Decided: unscheduled on purpose, gated on a trigger rather than a date**,
  on the owning-enum-payload precedent below. The trigger is the one §12 already names and this
  entry now writes down - **a collection of different types behind one interface, in Keel's own
  code**. Nothing has been one yet: a `Vector<T>` is homogeneous by construction, and this compiler
  does not count because it is written in C++. **Everything under the heading moves together and
  none of it is separable**: base classes, virtual methods, overriding, abstract bases, `protected`,
  and whatever `override` and `final` turn out to be. **Two things stay foreclosed and are stated
  here so they are not re-argued into the same slice**: RTTI and downcasting remain refused (§4), and
  a member *function* pointer stays an ordinary function pointer over `( ref C, args )` rather than
  growing a `this` adjustment and a vtable index - which is §12's own clause, and is true *only*
  while there is no dynamic dispatch, so that clause is the first thing to re-read the day this
  lands.
...
- **An enum cannot carry an owning payload.** D30 records the design - a synthesised destructor
  ...
  **M5.5 shipped without it, and this entry is how it stayed invisible (2026-09-24)**: "naturally
  M5.5's work" reads like a schedule and is not one, because M5.5's row never named it. **Decided:
  it stays refused, gated on a use case rather than a date.** The synthesised tag-switching
  destructor is real work and nothing has needed one yet; `Result<Buffer, E>` is the case to watch
  for, and M9's error handling is what would make it writable. Ship it when something asks.
...
- **A pointer to const has no spelling, and `[*]T` is when that stops being affordable** — **which is
  M8's first slice as of 2026-09-24**, so this is dated rather than pending. `const`
...
- **A raw pointer to a local may escape.** `i32* f() { i32 x = 1; return &x; }`
  ...
  *bindings* and correctly refuses `const ref i32 f() { i32 x = 1; return x; }`,
  ...
  `[*]T`**, which is when pointers stop being rare — **M8's first slice, as of 2026-09-24**. **The leaning is an error
...
- **`keelc` never links the runtime, so any program that allocates fails to link.** The driver's
  `cc` line names the generated `.c` and nothing else, and the only thing that supplies `kl_rt.c` is
  `run_tests.sh`, from a path it computes itself. So `alloc<T>()` works under the golden suite and
  under `--check`, and `keelc prog.kl -o prog` on the same source stops at *undefined reference to
  `kl_rt_alloc`*. Found on 2026-09-25 while writing `README.md`, whose opening sample is the first
  thing outside the suite to allocate. The fix is a line in the `cc` invocation, but *which* line is
  the real question - the suite compiles the runtime from source beside the output on purpose, and a
